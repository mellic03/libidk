...
get_descriptor(Vertex_POSITION3F_UV2F V)
{
    ...
    desc.attrib_size[0] = 3; // sizeof(V.position);
    desc.attrib_size[1] = 2; // sizeof(V.texcoords);

    desc.attrib_datatype[0] = GL_FLOAT;
    desc.attrib_datatype[1] = GL_FLOAT;

    size_t offset = 0;
    desc.attrib_offset[0] = offset;  offset += sizeof(V.position);
    desc.attrib_offset[1] = offset;  offset += sizeof(V.texcoords);
...
get_descriptor(Vertex_POSITION3F_NORMAL3F_TANGENT3F_UV2F V)
{
    ...
    desc.attrib_nbytes[idx] = sizeof(V.position);
    idx += 1;

    desc.attrib_size[idx]   = 3;
    desc.attrib_nbytes[idx] = sizeof(V.normal);
    idx += 1;

    desc.attrib_size[idx]   = 3;
    desc.attrib_nbytes[idx] = sizeof(V.tangent);
    idx += 1;

    desc.attrib_size[idx]   = 2;
    desc.attrib_nbytes[idx] = sizeof(V.texcoords);
    idx += 1;
